...
const Blog = () => {
    ...
    const handleDownload = async () => {
        ...
        try {
            // 1. Fetch the file as a "Blob" (Binary Data)
            ...
            link.href = url;
            link.download = `Verbatim_Dub_${selectedLang}_${Date.now()}.mp3`; // The filename
    ...
    const shareToSocial = (platform) => {
        const cleanText = blogContent.replace(/[#*]/g, '').substring(0, 2000);
        const shareUrl = "https://verbatim-project.vercel.app";
                                ...
                                <div className="space-y-6">
                                    ...
                                    {audioUrl && (
                                        <div className="mt-8 p-6 bg-black/40 rounded-2xl border border-green-500/30 text-center animate-in zoom-in-95">
                                            ...
                                            {/* --- FIXED: SMART DOWNLOAD BUTTON --- */}
                                            <button 
                                                onClick={handleDownload}
                                                disabled={isDownloading}
                                                className="flex items-center justify-center gap-2 w-full py-2 bg-white/5 hover:bg-green-500/20 text-green-400 border border-green-500/30 rounded-lg text-xs font-bold uppercase tracking-widest transition-all hover:scale-[1.02] cursor-pointer disabled:opacity-50"
                                            >
                                                {isDownloading ? (
                                                    <><span className="animate-spin rounded-full h-3 w-3 border-b-2 border-green-400"></span> Downloading...</>
                                                ) : (
